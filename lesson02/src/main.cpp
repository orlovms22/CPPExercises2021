...
void task2() {
    ...
    rassert(array2d.size() == 0, "Size of array2d is not 0!");
    ...
    row0.push_back(8);
    row0.push_back(10);
    rassert(row0.size(), "Size if row0 is not 3!");
    // TODO 22 добавьте rassert проверяющий размер этого вектора row0

    array2d.push_back(row0);
    rassert(array2d.size() == 1, "Size of array2d is not 1!");
...
//    // теперь давайте добавим еще один ряд чисел но другим способом:
    ...
    rassert(row1.size() == 3, "Size if row1 is not 3!");
    rassert(array2d.size() == 2, "Size if array2d is not 2!");
...
void task3() {
    ...
    int rows, cols;
    std::cout << "Please ener number of rows: ";
    std::cin >> rows;
    std::cout << "Please ener number of cols: ";
    std::cin >> cols;
    ...
    rassert(rows > 0 && rows <= 20, "Number of rows is not correct");
    rassert(cols > 0 && cols <= 20, "Number of cols is not correct");
    ...
    std::cout << "print2DArray(arr2d):" << std::endl;
    print2DArray(arr2d);
