...
void test1() {
    ...
    for (int i = 0; i < keypoints0.size(); ++i) {
        ...
        //лучше всего показал себя left-right check он отсеивает огромное число лишних точек, в два раза лучше чем остальные
        //можно использовать как left-right check отдельно так и с фильтрацией по порогу, однако после RANSAC результат один и тот же
        //использовать left-right check отдельно немного лучше, так как после ransac остается большее количество inlier точек
    ...
    img1to0 = img0.clone();
    cv::resize(img1_alternative, img1_alternative, img1.size());
